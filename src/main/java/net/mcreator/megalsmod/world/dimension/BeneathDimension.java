...
package net.mcreator.megalsmod.world.dimension;
...
import net.mcreator.megalsmod.item.BeneathItem;
...
public class BeneathDimension extends MegalsmodModElements.ModElement {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(Blocks.WATER.getDefaultState().getBlock()).build(), "field_222718_j");
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("ambient.underwater.loop"))),
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.PRISMARINE_BRICKS.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? Blocks.PRISMARINE_BRICKS.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
